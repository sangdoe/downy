...
const SliderControl: NextPage<SliderControlProps> = ({
  ...
  const goLeft = () => {
    if (
      slideableContainerRef.current &&
      sliderItemRef.current &&
      rightControlRef.current &&
      leftControlRef.current
    ) {
      let stopPosition;
      if (window.innerWidth >= 992) {
        if (window.innerWidth <= 1199) {
          stopPosition = -(currentSliderPosition.current,
          sliderItemRef.current?.clientWidth *
            (productsLength - alignmentPattern[0]) +
            15);
        } else if (window.innerWidth <= 1399) {
          stopPosition = -(currentSliderPosition.current,
          sliderItemRef.current?.clientWidth *
            (productsLength - alignmentPattern[1]) +
            15);
        } else if (window.innerWidth <= 1599) {
          stopPosition = -(currentSliderPosition.current,
          sliderItemRef.current?.clientWidth *
            (productsLength - alignmentPattern[2]) +
            15);
        } else {
          stopPosition = -(currentSliderPosition.current,
          sliderItemRef.current?.clientWidth *
            (productsLength - alignmentPattern[3]) +
            15);
        }
  ...
  const goRight = () => {
    if (
      ...
      if (window.innerWidth >= 992) {
        ...
        if (
          currentSliderPosition.current >=
          0 - sliderItemRef.current.clientWidth + 15
        ) {
